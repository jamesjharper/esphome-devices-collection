...
double Matrix3x3::determinant() const {
	return (
		(double)m[0][0] * (double)m[1][1] * (double)m[2][2] + 
...
Vec3 esphome::cms::matrices::operator*( const Vec3& a, const Matrix3x3& b ) {
	Vec3 r;
...
Vec3 esphome::cms::matrices::operator*( const Matrix3x3& b, const Vec3& a ) {
	Vec3	r;
